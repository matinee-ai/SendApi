...
from PySide6.QtWidgets import (
    ...
    QMessageBox, QGroupBox, QFormLayout, QLineEdit, QFileDialog
)
...
class Sidebar(QWidget):
    ...
    def init_ui(self):
        ...
        collections_label.setStyleSheet("font-weight: bold; font-size: 14px; color: #2c3e50; padding: 5px; background-color: #ecf0f1; border-radius: 3px;")
    ...
    def show_collection_context_menu(self, position):
        ...
        if isinstance(data, Collection):
            ...
            add_request_action.triggered.connect(lambda: self.add_request_to_collection(data))
            
            add_folder_action = context_menu.addAction("Add Folder")
            add_folder_action.triggered.connect(lambda: self.add_folder_to_collection(data))
            ...
            create_tests_action.triggered.connect(lambda: self.create_tests_for_collection(data))
            ...
            create_tests_action = context_menu.addAction("Create Tests")
            create_tests_action.triggered.connect(lambda: self.create_tests_for_request(data))
    ...
    def rename_collection(self, collection):
        """Rename a collection."""
        new_name, ok = QInputDialog.getText(self, "Rename Collection", "New name:", text=collection.name)
        if ok and new_name:
    ...
    def rename_request(self, request):
        """Rename a request."""
        new_name, ok = QInputDialog.getText(self, "Rename Request", "New name:", text=request.name)
        if ok and new_name:
    ...
    def export_collection(self, collection):
        ...
        if file_path:
            try:
                ...
                QMessageBox.information(self, "Success", f"Collection '{collection.name}' exported successfully!")
    ...
    def export_request_as_curl(self, request):
        ...
        try:
            ...
            if parent and hasattr(parent, '_generate_curl_command'):
                ...
                file_path, _ = QFileDialog.getSaveFileName(
                    self, "Save cURL Command", f"{request.name}.sh", "Shell Scripts (*.sh);;Text Files (*.txt);;All Files (*)"
                )
    ...
    def duplicate_collection(self, collection):
        ...
        try:
            ...
            duplicated_collection.id = f"{collection.id}_copy" if collection.id else f"{collection.name}_copy"
            ...
            QMessageBox.information(self, "Success", f"Collection '{collection.name}' duplicated successfully!")
    ...
    def run_all_requests(self, collection):
        ...
        if not all_requests:
            QMessageBox.information(self, "Info", "No requests found in this collection.")
            return
        
        # Ask for confirmation
        reply = QMessageBox.question(
            self, "Run All Requests", 
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            if parent and hasattr(parent, 'run_all_requests'):
                ...
                print(f"Sidebar: Could not find run_all_requests method in parent hierarchy")
                QMessageBox.warning(self, "Error", "Could not start batch testing. Please try again.")
        else:
    ...
    def _get_all_requests_from_collection(self, collection):
        ...
        # Add direct requests
        print(f"Sidebar: Collection '{collection.name}' has {len(collection.requests)} direct requests")
        requests.extend(collection.requests)
        
        # Add requests from nested folders
        print(f"Sidebar: Collection '{collection.name}' has {len(collection.folders)} folders")
        for folder in collection.folders:
            folder_requests = self._get_all_requests_from_collection(folder)
            print(f"Sidebar: Folder '{folder.name}' has {len(folder_requests)} requests")
    ...
    def create_tests_for_request(self, request):
        ...
        if self._has_standard_tests(request.tests):
            QMessageBox.information(self, "Info", f"Standard tests already exist in '{request.name}'")
            return
        ...
        QMessageBox.information(self, "Success", f"Standard tests added to '{request.name}'")
    ...
    def create_tests_for_collection(self, collection):
        ...
        requests_needing_tests = [req for req in all_requests if not self._has_standard_tests(req.tests)]
        
        if not requests_needing_tests:
            QMessageBox.information(self, "Info", f"All requests in '{collection.name}' already have standard tests.")
            return
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            QMessageBox.information(self, "Success", f"Standard tests added to {updated_count} requests in '{collection.name}'")
