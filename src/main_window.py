...
from PySide6.QtWidgets import (
    ...
    QLabel, QComboBox, QLineEdit, QTextEdit, QTableWidget,
    QTableWidgetItem, QHeaderView, QMessageBox, QFileDialog,
    QMenu, QInputDialog, QGroupBox, QFormLayout, QCheckBox, QProgressBar
)
...
class MainWindow(QMainWindow):
    ...
    def show_about(self):
        """Show about dialog."""
        QMessageBox.about(self, "About SendApi", 
                         "SendApi v1.0\n\n"
    ...
    def import_file(self):
        ...
        if file_path:
            try:
                ...
                imported_collections, imported_environments = PostmanImporter.import_file(file_path)
                ...
                if imported_collections:
                    if len(imported_collections) == 1:
                        success_messages.append(f"Collection '{imported_collections[0].name}'")
                    else:
                        success_messages.append(f"{len(imported_collections)} collections")
                
                if imported_environments:
                    if len(imported_environments) == 1:
                        success_messages.append(f"Environment '{imported_environments[0].name}'")
                    else:
                        success_messages.append(f"{len(imported_environments)} environments")
                
                if success_messages:
                    message = f"Imported: {', '.join(success_messages)}"
                    QMessageBox.information(self, "Success", message)
                else:
                    QMessageBox.warning(self, "Warning", "No collections or environments found in the file.")
    ...
    def import_environment(self):
        ...
        if file_path:
            try:
                ...
                # Show success message
                if len(imported_environments) == 1:
                    QMessageBox.information(self, "Success", f"Environment '{imported_environments[0].name}' imported successfully!")
                else:
                    QMessageBox.information(self, "Success", f"{len(imported_environments)} environments imported successfully!")
    ...
    def save_request_as_curl(self):
        ...
        try:
            ...
            file_path, _ = QFileDialog.getSaveFileName(
                self, "Save cURL Command", f"{current_request.name}.sh", "Shell Scripts (*.sh);;Text Files (*.txt);;All Files (*)"
            )
            
            if file_path:
                with open(file_path, 'w', encoding='utf-8') as f:
                    f.write(curl_command)
                QMessageBox.information(self, "Success", f"cURL command saved to {file_path}")
    ...
    def save_response(self):
        ...
        try:
            ...
            # Ask user for file path
            file_path, _ = QFileDialog.getSaveFileName(
                self, "Save Response", "response.txt", "Text Files (*.txt);;JSON Files (*.json);;All Files (*)"
            )
            
            if file_path:
                with open(file_path, 'w', encoding='utf-8') as f:
                    f.write(response_body)
                QMessageBox.information(self, "Success", f"Response saved to {file_path}")
    ...
    def export_collection(self):
        ...
        if file_path:
            try:
                ...
                QMessageBox.information(self, "Success", "Collection exported successfully!")
            except Exception as e:
    ...
    def on_send_request(self, request_data):
        ...
        print(f"Sending request: {request_data.get('method', 'GET')} {request_data.get('url', 'No URL')}")
        ...
        self.request_runner.response_received.connect(self.response_panel.display_response)
    ...
    def run_all_requests(self, requests):
        ...
        current_environment = self.environment_panel.get_current_environment()
        print(f"MainWindow: Using environment: {current_environment.name if current_environment else 'None'}")
    ...
    def start_batch_test(self):
        ...
        if not self.current_batch_requests:
            QMessageBox.information(self, "Info", "No requests selected for batch testing. Please run 'Run All Requests' from a collection first.")
            return
    ...
    def on_batch_request_completed(self, request, response, test_results):
        ...
        print(f"MainWindow: Batch request completed: {request.name} - Status: {response.get('status_code', 'Unknown')}")
        ...
        for row in range(self.results_table.rowCount()):
            if self.results_table.item(row, 0).text() == request.name:
                ...
                self.results_table.setItem(row, 2, QTableWidgetItem(f"{response_time:.2f} ms"))
                break
        
        # Update progress
        current_progress = self.progress_bar.value() + 1
        self.progress_bar.setValue(current_progress)
        self.progress_label.setText(f"Completed {current_progress}/{self.progress_bar.maximum()} requests")
    ...
    def on_batch_all_completed(self, results):
        ...
        # Update progress
        self.progress_label.setText(f"Batch testing completed! {len(results)} requests processed.")
        ...
        QMessageBox.information(self, "Batch Run Complete", 
    ...
    def create_batch_testing_widget(self):
        ...
        header_label.setStyleSheet("font-weight: bold; font-size: 16px; color: #2c3e50; padding: 10px; background-color: #ecf0f1; border-radius: 5px;")
        ...
        self.results_table.setHorizontalHeaderLabels(["Request", "Status & Tests", "Response Time"])
        ...
        self.summary_label.setStyleSheet("font-weight: bold; font-size: 12px; color: #2c3e50; padding: 5px;")
